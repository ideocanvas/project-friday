...
TTS_MODEL = os.environ.get("TTS_MODEL", "Qwen/Qwen3-4B")
TTS_VOICE = os.environ.get("TTS_VOICE", "af_heart")
TTS_LANG_CODE = os.environ.get("TTS_LANG_CODE", "en")  # 'en' for English, 'zh' for Chinese
...
def transcribe_audio(audio_path: str, model: str = None, smart_split: bool = False) -> Dict[str, Any]:
    """
...
def speak_text(text: str, voice: str = None, lang_code: str = None, model: str = None) -> Dict[str, Any]:
    """
...
def logic(params: dict, user_id: str) -> dict:
    ...
    if action == "status":
        ...
        if status["mlx_available"] and status["mlx_whisper_available"] and status["mlx_audio_available"]:
            metal_status = "✅ Metal acceleration enabled" if status["metal_available"] else "⚠️ Using CPU (slower)"
            return {
                "success": True,
