...
# English reference audio for non-Chinese TTS
ENGLISH_REF_AUDIO = os.path.join(SCRIPT_DIR, "references", "speaker_en.wav")
ENGLISH_REF_TEXT = "Yeah, he also wants to change the globe haha, looks like I accumulated 100 happiness points for you, do you want to hear 2 more jokes?"
...
class TTSEngine:
    ...
    def is_mlx_audio_available(cls) -> Tuple[bool, str]:
        ...
        try:
            ...
            return (
                False,
                "MLX-Audio TTS not installed. Install with: pip install mlx-audio",
            )
    ...
    def generate(
        ...
        try:
            ...
            if use_qwen3:
                ...
                # Select reference based on language code
                is_chinese = lang_code in ["z", "zh", "chinese"]

                if is_chinese:
                    # Use Chinese reference for Chinese language
                    actual_ref_audio = ref_audio or TTS_REF_AUDIO
                    actual_ref_text = ref_text or TTS_REF_TEXT
                else:
                    # Use English reference for non-Chinese languages (better number pronunciation)
                    if ref_audio:
                        # User provided custom reference - use it
                        actual_ref_audio = ref_audio
                        actual_ref_text = ref_text or ENGLISH_REF_TEXT
                    else:
                        # Use built-in English reference
                        actual_ref_audio = ENGLISH_REF_AUDIO
                        actual_ref_text = ENGLISH_REF_TEXT

                # Validate reference audio exists
                if not os.path.exists(actual_ref_audio):
                    # Try to find in references directory
                    ref_in_dir = os.path.join(
                        SCRIPT_DIR, "references", os.path.basename(actual_ref_audio)
                    )
                    if os.path.exists(ref_in_dir):
                        actual_ref_audio = ref_in_dir
                    else:
                        # Fall back to language-appropriate default reference
                        if is_chinese:
                ...
                if actual_ref_text.endswith(".txt"):
                    txt_path = (
                        os.path.join(SCRIPT_DIR, "references", actual_ref_text)
                        if not os.path.isabs(actual_ref_text)
                        else actual_ref_text
                    )
