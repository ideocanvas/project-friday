...
# Configuration
MAX_SOURCES_DEFAULT = int(os.getenv('DEEP_RESEARCH_MAX_SOURCES', '5'))
...
class ResearchResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.page_url:
            ...
            result['message'] = f"🔬 Research complete ({self.sources_visited} sources, {self.duration_ms}ms)\n\n{self.summary}\n\n📄 Full report: {self.page_url}"
        else:
            result['message'] = f"🔬 Research complete ({self.sources_visited} sources, {self.duration_ms}ms)\n\n{self.summary}"
...
class DeepResearcher:
    ...
    def __init__(self, query: str, mode: str = 'deep', max_sources: int = MAX_SOURCES_DEFAULT):
        self.query = query
        self.mode = mode
        self.max_sources = min(max_sources, 2) if mode == 'quick' else min(max_sources, MAX_SOURCES_DEFAULT)
        self.findings: List[Dict[str, Any]] = []
    ...
    async def research(self) -> ResearchResult:
        ...
        try:
            ...
            while iteration < MAX_ITERATIONS and len(self.findings) < self.max_sources:
                iteration += 1
                print(f"[Researcher] Iteration {iteration}/{MAX_ITERATIONS}, sources: {len(self.findings)}/{self.max_sources}")
                ...
                if search_type == 'news':
                    search_result = search(current_search, num_results=5, date_range=date_range or 'week')
                else:
                ...
                for idx in ranked_indices:
                    ...
                    if not page_data.get('success'):
                        self.errors.append(f"Failed to browse {url}: {page_data.get('error', 'Unknown')}")
                        continue
                    ...
                    if extracted:
                        ...
                        if extracted.get('has_answer'):
                            ...
                            if evaluation and evaluation.get('is_sufficient'):
                                print(f"[Researcher] Findings sufficient after {len(self.findings)} sources")
                                break
    ...
    async def analyze_url(self, url: str, query: Optional[str] = None) -> ResearchResult:
        """
        ...
        try:
            ...
            if not page_data.get('success'):
                return self._error_result(f"Failed to browse {url}: {page_data.get('error', 'Unknown')}")

            # Extract info
            focus_areas = [query] if query else ["main content", "key points", "important information"]
            extracted = self._extract_info(
                page_data.get('title', ''),
                page_data.get('text', ''),
                page_data.get('url', url),
            ...
            if extracted:
                ...
                if (not extracted.get('has_answer') and
                        extracted.get('missing_info') and
                        page_data.get('links')):
    ...
    async def _follow_child_links(self, links: List[Dict[str, Any]],
                                   missing_info: List[str], query: str,
                                   max_additional: int = 2):
        """Follow promising child links to gather missing information."""
        # Use LLM to pick the best links to follow
        links_text = "\n".join(
            f"- [{l.get('text', 'Untitled')}]({l.get('href', '')})"
            for l in links[:20]
        )
...
Return ONLY the JSON array."""
    ...
    def _plan_research(self) -> Optional[Dict[str, Any]]:
        ...
        return call_llm_json(prompt, system_msg="You are a research planning assistant.",
    ...
    def _rank_urls(self, search_results: List[Dict[str, Any]]) -> List[int]:
        ...
        result = call_llm_json(prompt, system_msg="You are a search result ranking assistant.",
                               temperature=0.2, max_tokens=300)
        if result and 'ranked_indices' in result:
            return result['ranked_indices']
        return []

    def _extract_info(self, page_title: str, page_text: str, url: str,
                      focus_areas: List[str]) -> Optional[Dict[str, Any]]:
        ...
        return call_llm_json(prompt, system_msg="You are a research information extraction assistant.",
                             temperature=0.2, max_tokens=1024)

    def _evaluate_sufficiency(self, iteration: int) -> Optional[Dict[str, Any]]:
        """Use LLM to evaluate if findings are sufficient."""
        prompt = evaluate_sufficiency_prompt(self.query, self.findings, iteration, MAX_ITERATIONS)
        return call_llm_json(prompt, system_msg="You are a research evaluation assistant.",
                             temperature=0.3, max_tokens=500)
    ...
    def _synthesize(self) -> str:
        ...
        return call_llm(prompt, system_msg="You are a research synthesis assistant. Write clear, informative answers.",
